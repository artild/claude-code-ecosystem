...
import { NODES, EDGES, CX, CY, R1, R2, POS, SZ, SZ_HOVER, SZ_ACTIVE, FONT_SZ, crv } from "./data";
  ...
  return (
    <svg viewBox="-160 -40 1320 1080" role="img" aria-label="Claude Code ecosystem mind map" style={{ width: "100%", display: "block" }}>
      ...
      {NODES.map(n => { const p = POS[n.id], on = activeId === n.id, hv = hoveredId === n.id;
        return <line key={`s-${n.id}`} x1={CX} y1={CY} x2={p.x} y2={p.y} stroke={n.color} strokeWidth={on ? 1 : .4} opacity={on ? .15 : hv ? .08 : .02} style={{ transition: "all .3s" }} />;
      })}
      {/* Edges */}
      ...
      {NODES.map(n => { const p = POS[n.id], on = activeId === n.id, hv = hoveredId === n.id;
        ...
        return (<g key={n.id} role="button" aria-label={n.title} tabIndex={0} style={{ cursor: "pointer", outline: "none" }}
          ...
          <circle cx={p.x} cy={p.y} r={sz} fill={n.color} opacity={on ? 1 : hv ? .95 : linked ? .85 : n.tier === 1 ? .8 : .65} style={{ transition: "all .25s" }} />
          <text x={p.x} y={p.y + 1} textAnchor="middle" dominantBaseline="central" fontSize={fontSize} fontWeight="600" fill="#fff" opacity=".95" style={{ pointerEvents: "none", letterSpacing: -.3 }}>{n.short}</text>
          <text x={lx} y={ly} textAnchor={anch} dominantBaseline="central" fontSize={labelFontSize} fontWeight={on ? "700" : linked ? "600" : "500"} fill="var(--fg)" opacity={on ? 1 : (hv || linked) ? .7 : n.tier === 1 ? .5 : .35} style={{ pointerEvents: "none", transition: "all .3s" }}>{n.title}</text>
        </g>);
      })}
    </svg>
  );
