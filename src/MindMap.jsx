...
export default function MindMap({ activeId, hoveredId, showEdges, onSelect, onHover }) {
  ...
  return (
    <svg viewBox="0 0 800 800" role="img" aria-label="Claude Code ecosystem mind map" style={{ width: "100%", display: "block" }}>
      ...
      {showEdges && EDGES.map((e, i) => { const { d, cpx, cpy } = crv(e.from, e.to); const col = NODES.find(n => n.id === e.from)?.color || "#888"; const hot = litIdx.includes(i) || hovEdge === i;
        return (<g key={`e-${i}`}>
          ...
          {hot && <g style={{ pointerEvents: "none" }}><rect x={cpx - 68} y={cpy - 13} width={136} height={26} rx={8} fill="var(--tooltip-bg)" stroke={col} strokeWidth={e.solid ? 1.2 : .6} /><text x={cpx} y={cpy + 1} textAnchor="middle" dominantBaseline="central" fontSize="10" fontWeight="600" fill={col}>{e.label}</text></g>}
        </g>);
      })}
      <circle cx={CX} cy={CY} r={44} fill="var(--fg)" opacity="0.035" />
      <text x={CX} y={CY - 7} textAnchor="middle" fontSize="16" fontWeight="700" fill="var(--fg)" opacity="0.6" style={{ letterSpacing: -.5 }}>Claude</text>
      <text x={CX} y={CY + 12} textAnchor="middle" fontSize="12" fontWeight="500" fill="var(--fg)" opacity="0.28">Code</text>
      {NODES.map(n => { const p = POS[n.id], on = activeId === n.id, hv = hoveredId === n.id;
        ...
        return (<g key={n.id} role="button" aria-label={n.title} tabIndex={0} style={{ cursor: "pointer", outline: "none" }}
          ...
          <text x={lx} y={ly} textAnchor={anch} dominantBaseline="central" fontSize="11.5" fontWeight={on ? "700" : linked ? "600" : "400"} fill="var(--fg)" opacity={on ? .88 : (hv || linked) ? .55 : .28} style={{ pointerEvents: "none", transition: "all .3s" }}>{n.title}</text>
        </g>);
      })}
    </svg>
  );
