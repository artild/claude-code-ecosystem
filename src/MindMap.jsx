    ...
    <svg viewBox="-160 -40 1320 1080" role="img" aria-label="Claude Code ecosystem mind map" style={{ width: "100%", display: "block" }}>
      ...
      {NODES.map(n => { const p = POS[n.id], on = activeId === n.id, hv = hoveredId === n.id;
        ...
        return (<g key={n.id} role="button" aria-label={n.title} tabIndex={0} style={{ cursor: "pointer", outline: "none" }}
          ...
          <circle cx={p.x} cy={p.y} r={sz} fill={n.color} opacity={on ? 1 : hv ? .95 : linked ? .85 : .75} style={{ transition: "all .25s" }} />
          ...
          <text x={lx} y={ly} textAnchor={anch} dominantBaseline="central" fontSize="14" fontWeight={on ? "700" : linked ? "600" : "500"} fill="var(--fg)" opacity={on ? 1 : (hv || linked) ? .7 : .45} style={{ pointerEvents: "none", transition: "all .3s" }}>{n.title}</text>
        </g>);
      })}
    </svg>
  );
